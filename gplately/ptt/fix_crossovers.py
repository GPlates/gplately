...
def _fix_crossovers(
    ...
    if ignore_moving_plates:
        crossover_filter = lambda crossover: crossover.moving_plate_id not in ignore_moving_plates
    ...
    if debug:
        ...
        for crossover_result in crossover_results:
            ...
            elif crossover.type == pygplates.CrossoverType.synch_young_crossover_and_stages:
            ...
            print('Time({0}), moving_pid({1}), young_fixed_pid({2}), old_fixed_pid({3}), type({4}): {5}'.format(
                    crossover.time,
...
if __name__ == "__main__":
    ...
    __description__ = \
    """Loads one or more input rotation files, fixes any crossovers and saves the rotations to \
output rotation files.
    ...
    python %(prog)s -d -c 0.01 -i 201 701 -- input_rotations1.rot input_rotations2.rot
    ...
    parser.add_argument('-c', '--crossover_threshold_degrees', type=parse_positive_number,
    ...
    crossover_type_group.add_argument('-x', '--default_xo_ys', action="store_true",
    ...
    rotation_feature_collections = [file_registry.read(input_rotation_filename)
    ...
    if args.crossover_type_default_xo_ys:
        ...
        crossover_type_function = pygplates.CrossoverTypeFunction.type_from_xo_tags_in_comment_default_xo_ig
    else:
        crossover_type_function = pygplates.CrossoverTypeFunction.type_from_xo_tags_in_comment
    ...
    if not _fix_crossovers(
            ...
            args.debug):
        print('Warning: One or more crossovers were not processed since unable to determine crossover '
    ...
    for rotation_feature_collection_index in range(len(rotation_feature_collections)):
        ...
        output_rotation_filename = ''.join((filename_root, args.output_filename_suffix, filename_ext))
