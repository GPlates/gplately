...
def call_system_command(
    ...
    **subprocess_options  # Advanced options passed directly to subprocess.Popen().
    ...
    try:
        ...
        if print_errors:
            print(
                "System command called with invalid arguments: {0}".format(e),
                file=sys.stderr,
            )
        if not raise_errors:
            return None
        ...
        if print_errors:
            print(
                "Unable to execute system command: {0} {1}".format(args, e),
                file=sys.stderr,
            )
        if not raise_errors:
            return None
    ...
    if check_return_code is not None:
        ...
        if command_return_code != check_return_code:
            if print_errors:
                print(
                    "System command failed: {0} return code: {1}".format(
                        args, command_return_code
                    ),
                    file=sys.stderr,
                )
            if not raise_errors:
            ...
            raise subprocess.CalledProcessError(
                command_return_code, args, output=stdout
            )
