...
def filterGPML(**kwargs):
        ...
        if parameter in filterProperties:
            if parameter == filterProperties[0]:
                ...
                if ageExistsWindow[1] > ageExistsWindow[0]:
                    print(" ")
                    print(
                        "ERROR - Age exists window end age older than begin age: "
                        + str(ageExistsWindow[1])
                    )
                ...
                if pgp.LatLonPoint.is_valid_longitude(boundingBox[0]) is False:
                    print(" ")
                    print(
                        "ERROR - Bounding box longitude is not valid: "
                        + str(boundingBox[0])
                    )
                ...
                if pgp.LatLonPoint.is_valid_latitude(boundingBox[2]) is False:
                    print(" ")
                    print(
                        "ERROR - Bounding box latitude is not valid: "
                        + str(boundingBox[2])
                    )
    ...
    if os.path.isfile("output/output.gpml"):
        ...
        print("Housekeeping:")
        print(
            "    Previous 'output.gpml' found in destination folder. File removed for new filter sequence."
        )
    ...
    for filter_ in filterSequence:
        ...
        if filter_ == 1:
                                ...
                                if filter_property.get_name() == "conjugatePlateId":
                                    ...
                                    if inverse == False:
                                        ...
                                        if str(selected_filter_property) == str(
                                            rPlateID[0]
                                        ) and str(second_filter_property) == str(
                                            cPlateID[0]
                                        ):
                                        ...
                                        if (
                                            int(str(selected_filter_property))
                                            not in rPlateID
                                            or int(str(second_filter_property))
                                            not in cPlateID
                                        ):
            ...
            if cascade == False:
                ...
                print(" ")
                print(
                    "    1. Filtering data by reconstruction plate ID: "
                    + str(rPlateID)
                    + " and conjugate plate ID: "
                    + str(cPlateID)
                )
                ...
                print(" ")
                print(
                    "    1. Filtering data by reconstruction plate ID(s): "
                    + str(rPlateID)
                )
                print("       - Found " + str(len(f1_result)) + " feature(s).")
        ...
        if filter_ == 3:
            ...
            for feature in data_:
                ...
                if (
                    begin_time <= ageAppearWindow[0]
                    and begin_time >= ageAppearWindow[1]
                ):
                    f3_result.add(feature)

            print(
                "    3. Filtering data by age of appearance window: "
                + str(ageAppearWindow[0])
                + " - "
                + str(ageAppearWindow[1])
                + " Ma"
            )
        ...
        if filter_ == 4:
            ...
            for feature in data_:
                ...
                if (
                    end_time <= ageDisappearWindow[0]
                    and end_time >= ageDisappearWindow[1]
                ):
                    f4_result.add(feature)

            print(
                "    4. Filtering data by age of disappearance window: "
                + str(ageDisappearWindow[0])
                + " - "
                + str(ageDisappearWindow[1])
                + " Ma"
            )
        ...
        if filter_ == 5:
            ...
            for feature in data_:
                for property in feature:
                    ...
                    if filter_property.get_name() == "centerLineOf":
                        ...
                        points = (
                            selected_filter_property.get_value()
                            .get_base_curve()
                            .get_polyline()
                            .get_points_view()
                        )
                        ...
                        for point in points:
                            point_latlong = (
                                pgp.convert_point_on_sphere_to_lat_lon_point(point)
                            )

                            if (
                                point_latlong.get_longitude() >= float(boundingBox[0])
                                and point_latlong.get_longitude()
                                <= float(boundingBox[1])
                                and point_latlong.get_latitude()
                                >= float(boundingBox[2])
                                and point_latlong.get_latitude()
                                <= float(boundingBox[3])
                            ):
            ...
            print(
                "    5. Filtering data by geographic bounding box: "
                + str(boundingBox[0])
                + "/"
                + str(boundingBox[1])
                + "/"
        ...
        if filter_ == 6:
            ...
            for feature in data_:
                ...
                if begin_time >= ageExistsWindow[0] and end_time <= ageExistsWindow[1]:
                    ...
                    begin_time >= ageExistsWindow[0]
                    and end_time <= ageExistsWindow[0]
                    and end_time >= ageExistsWindow[1]
                ):
                    f6_result.add(feature)
                elif (
                    begin_time <= ageExistsWindow[0] and end_time >= ageExistsWindow[1]
                ):
                    f6_result.add(feature)
                elif (
                    begin_time <= ageExistsWindow[0]
            ...
            print(
                "    6. Filtering data by age of existence window: "
                + str(ageExistsWindow[0])
                + " - "
                + str(ageExistsWindow[1])
                + " Ma"
            )
        ...
        if filter_ == 8:
            ...
            print(
                "    8. Filtering data by feature geometries present: "
                + str(geometryType)
            )
            ...
            if "MultiPointOnSphere" in geometryType:
                print(
                    "       - Found " + str(multiPointCount) + " MultiPointOnSphere(s)."
                )
        ...
        if filter_ == 11:
            ...
            for feature in data_:
                ...
                if (
                    begin_time > feature_truncate_age
                ...
                if (
                    ...
                    if str(feature.get_feature_type()) == "gpml:SubductionZone":
                        ...
                        if "gpml:subductionZoneAge" not in sz_age_array:
                            ...
                            # print(feature.get_name())
                            feature.add(
                                pgp.PropertyName.create_gpml("subductionZoneAge"),
            ...
            print(
                "    11. File truncated by age boundary: "
                + str(feature_truncate_age)
                + " Ma"
            )
            print(
                "       - Created "
                + str(len(f11a_result))
                + " feature(s) older than truncation boundary."
            )
            print(
                "       - Created "
                + str(len(f11b_result))
                + " feature(s) younger than truncation boundary."
            )
            print(" ")
    ...
    if previousFilter == 11:
        ...
        if len(iso_output1) != 0:
            ...
            outputFeatureCollection1.write(
                iso_output1,
                "output/>" + str(feature_truncate_age) + "Ma_" + str(outputFile),
            )
