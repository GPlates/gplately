    ...
    def create_initial_ocean_seed_points(self):
        ...
        * inside the polygons (for the continental mask)
        ...
        if (
            ...
            # ✅ New prompt asking user to choose between continent contouring or COB
            if not self.continent_contouring and not hasattr(self, 'continent_contouring_prompted'):
                ...
                user_input = input(
                    "No COB masking file found.\n"
                    "If you want to proceed with default age gridding using COBs generated by 'continent contouring', select 'yes'.\n"
                    "If you want to use an existing COB file, select 'no' and provide a COB filename later.\n"
                    "(yes/NO): "
                ).strip().lower()
                ...
                if user_input == 'yes':
                    ...
                    raise RuntimeError(
                        "Continent contouring was not approved. Please supply a COB masking file and rerun."
                    )
                else:
                    raise RuntimeError(
            ...
            elif self.continent_contouring:
                print("Proceeding with continent contouring as requested.")
                ocean_points = self._generate_ocean_points()
            else:
                raise RuntimeError(
