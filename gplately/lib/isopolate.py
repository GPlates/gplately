...
DEFAULT_INTERPOLATE_RESOLUTION_RADIANS = math.radians(
...
DEFAULT_MINIMUM_LATITUDE_OVERLAP_RADIANS = math.radians(
...
DEFAULT_MAXIMUM_ISOCHRON_LATITUDE_NON_OVERLAP_RADIANS = math.radians(
    DEFAULT_MAXIMUM_ISOCHRON_LATITUDE_NON_OVERLAP_DEGREES
)
# For COBs...
DEFAULT_MAXIMUM_COB_LATITUDE_NON_OVERLAP_DEGREES = 1
DEFAULT_MAXIMUM_COB_LATITUDE_NON_OVERLAP_RADIANS = math.radians(
...
DEFAULT_MAXIMUM_DISTANCE_THRESHOLD_RADIANS = math.radians(
...
DEFAULT_JOIN_POLYLINES_THRESHOLD_RADIANS = math.radians(
...
# Scalar types.
SCALAR_TYPE_SPREADING_ASYMMETRY = pygplates.ScalarType.create_gpml(
    SCALAR_COVERAGE_SPREADING_ASYMMETRY
)
SCALAR_TYPE_SPREADING_RATE = pygplates.ScalarType.create_gpml(
    SCALAR_COVERAGE_SPREADING_RATE
)
SCALAR_TYPE_FULL_SPREADING_RATE = pygplates.ScalarType.create_gpml(
    SCALAR_COVERAGE_FULL_SPREADING_RATE
)
SCALAR_TYPE_SPREADING_DIRECTION = pygplates.ScalarType.create_gpml(
    SCALAR_COVERAGE_SPREADING_DIRECTION
)
SCALAR_TYPE_SPREADING_OBLIQUITY = pygplates.ScalarType.create_gpml(
    ...
    for feature in features:
        ...
        if feature_end_time > feature_begin_time:
            if print_debug_output >= 1:
                print(
                    " ...skipping '{0}' "
                    "feature (id: {1}): begin time {2} later than end time {3}.".format(
...
def _calc_spreading_asymmetries(
    ...
    stage_rotation_vector_for_present_day_geometry = pygplates.Vector3D(
        stage_rotation_pole_for_present_day_geometry.to_xyz()
    )
    ...
    for point_index in range(len(young_polyline)):
        ...
        young_vector_perp_pole = pygplates.Vector3D.cross(
            pygplates.Vector3D.cross(
                ...
                young_polyline[point_index].to_xyz(),
            ),
            stage_rotation_vector_for_present_day_geometry,
        )
        ...
        old_vector_perp_pole = pygplates.Vector3D.cross(
            pygplates.Vector3D.cross(
                stage_rotation_vector_for_present_day_geometry,
                old_polyline[point_index].to_xyz(),
            ),
            stage_rotation_vector_for_present_day_geometry,
        )
        ...
        try:
            angle_between_young_and_old_points = pygplates.Vector3D.angle_between(
                young_vector_perp_pole, old_vector_perp_pole
            )
        ...
        asymmetry = (
            2
            * min(angle_between_young_and_old_points / abs_stage_pole_angle_radians, 1)
            - 1
        )
...
def _add_tessellated_scalar_coverages(
    ...
    if tessellate_threshold_radians:
        ...
        for segment_index in range(len(segments)):
            ...
            tessellated_segment_points = segment.to_tessellated(
                tessellate_threshold_radians
            )
    ...
    if (
        SCALAR_TYPE_SPREADING_RATE in output_scalar_types
        or SCALAR_TYPE_FULL_SPREADING_RATE in output_scalar_types
        or SCALAR_TYPE_SPREADING_DIRECTION in output_scalar_types
        or SCALAR_TYPE_SPREADING_OBLIQUITY in output_scalar_types
    ):
        ...
        #
        #
        interpolated_isochron_reconstruction = (
            rotation_model.get_rotation(
                interpolated_time, interpolated_isochron_plate_id
            )
            * inverse_present_day_rotation
        )
        full_spreading_velocity_vectors = [
            interpolated_isochron_reconstruction * velocity
            for velocity in full_spreading_velocity_vectors_at_present_day
        ]
        reconstructed_interpolated_points = [
            interpolated_isochron_reconstruction * point
        ...
        full_spreading_velocities = pygplates.LocalCartesian.convert_from_geocentric_to_magnitude_azimuth_inclination(
            reconstructed_interpolated_points, full_spreading_velocity_vectors
        )
        ...
        if SCALAR_TYPE_SPREADING_RATE in output_scalar_types:
            ...
            spreading_rates = [
                0.5
                * (spreading_asymmetries[point_index] + 1)
                * full_spreading_velocities[point_index][0]
        ...
        if SCALAR_TYPE_FULL_SPREADING_RATE in output_scalar_types:
            full_spreading_rates = [
                velocity[0] for velocity in full_spreading_velocities
            ]
        ...
        if SCALAR_TYPE_SPREADING_OBLIQUITY in output_scalar_types:
            ...
            for point_index in range(num_points):
                ...
                velocity_vector = full_spreading_velocity_vectors_at_present_day[
                    point_index
                ]
                ...
                if prev_normal:
                    # Range [0,180].
                    spreading_obliquity_for_prev_normal = math.degrees(
                        pygplates.Vector3D.angle_between(velocity_vector, prev_normal)
                    )
                    # Range [0,90].
                    if spreading_obliquity_for_prev_normal > 90:
                        spreading_obliquity_for_prev_normal = (
                            180 - spreading_obliquity_for_prev_normal
                        )
                ...
                if next_normal:
                    # Range [0,180].
                    spreading_obliquity_for_next_normal = math.degrees(
                        pygplates.Vector3D.angle_between(velocity_vector, next_normal)
                    )
                    # Range [0,90].
                    if spreading_obliquity_for_next_normal > 90:
                        spreading_obliquity_for_next_normal = (
                            180 - spreading_obliquity_for_next_normal
                        )
                    # Exclude transform sections (obliquity larger than 45 degrees).
                ...
                if (
                    spreading_obliquity_for_prev_normal is not None
                    and spreading_obliquity_for_next_normal is not None
                ):
                    # Both previous and next normals have ridge-like spreading obliquities.
                    spreading_obliquity = 0.5 * (
                        spreading_obliquity_for_prev_normal
                        + spreading_obliquity_for_next_normal
                    )
    ...
    if SCALAR_TYPE_AGE in output_scalar_types:
        ...
        scalar_coverages[SCALAR_TYPE_AGE] = [interpolated_time] * len(
            interpolated_polyline
        )
...
def _write_coverage_to_xy_file(xy_file, coverage, output_scalar_types):
    ...
    line_format_string = " ".join(
        ["{{{0}}}".format(i) for i in range(len(scalars_list))]
    )
    line_format_string += "\n"
...
def join_adjacent_features_with_same_type_and_begin_time_and_plate_ids(
    features, distance_threshold_radians, print_debug_output
):
        ...
        if not left_right_plate_ids:
            if print_debug_output >= 1:
                print(
                    ...
                    "'gpml:OldPlatesHeader' property.".format(
                        feature.get_feature_type(), feature.get_feature_id()
                    )
                )
            continue
    ...
    for join_feature_group in join_feature_groups.values():
        ...
        joined_features.extend(
            _join_adjacent_features(
                join_feature_group, distance_threshold_radians, print_debug_output
            )
        )
...
def interpolate_isochrons(
    ...
    *maximum_cob_latitude_non_overlap_radians* and *maximum_distance_threshold_radians*
    ...
    isochron_cob_ridge_features = pygplates.FeaturesFunctionArgument(
        isochron_cob_ridge_features
    )
    ...
    joined_isochron_cob_ridge_features = (
        join_adjacent_features_with_same_type_and_begin_time_and_plate_ids(
            isochron_cob_ridge_features,
    ...
    if print_debug_output >= 1:
        if using_interpolation_times:
            print(
                "Interpolate isochrons at geological times {0}...".format(
                    interpolation_times
                )
            )
        else:
            print(
                "Interpolate isochrons at {0} minute distance intervals...".format(
    ...
    for feature in joined_isochron_cob_ridge_features:
        ...
        if not left_right_plate_ids:
            ...
            if print_debug_output >= 1:
                print(
                    " ...skipping '{0}' feature (id: {1}): No left/right or "
                    "reconstruction/conjugate plate IDs and no 'gpml:OldPlatesHeader' property.".format(
                        feature.get_feature_type(), feature.get_feature_id()
                    )
                )
            continue
        ...
        if feature.get_feature_type() == MID_OCEAN_RIDGE_FEATURE_TYPE:
            feature_groups.setdefault((right_plate_id, left_plate_id), []).append(
                feature
            )
    ...
    for feature_group in feature_groups.values():
        feature_group.sort(
            key=lambda feature: (
                ...
                feature.get_feature_type() == COB_FEATURE_TYPE,
            )
        )
    ...
    for (left_plate_id, right_plate_id), feature_group in feature_groups.items():
        if print_debug_output >= 1:
            print(
                " Interpolating left plate {0}, right plate {1}".format(
        ...
        inverse_present_day_rotation_for_interpolated_isochrons = (
            rotation_model.get_rotation(0, left_plate_id).get_inverse()
        )
        ...
        for old_feature_index in range(
            len(feature_group) - 1, 0, -1
        ):  # Note: excludes index 0
            ...
            if using_interpolation_times:
                ...
                if not interpolation_times or interpolation_times[0] > old_time:
                    break
            ...
            present_day_rotation_for_old_feature = rotation_model.get_rotation(
                0, old_feature.get_reconstruction_plate_id()
            )
            # Note that we don't use 'pygplates.reconstruct' since that excludes features that don't exist at present day.
            old_feature_geoms = [
                present_day_rotation_for_old_feature * geom
                for geom in old_feature_geoms
            ]
            ...
            while young_feature_index >= 0:
                ...
                if young_feature.get_feature_type() != COB_FEATURE_TYPE:
                    young_feature_begin_time, young_feature_end_time = (
                        young_feature.get_valid_time()
                    )
            ...
            if young_feature_index < 0:
                if print_debug_output >= 2:
                    print(
                        "   ...skipping old feature - unable to find a younger, non-COB feature"
                    )
                continue
            ...
            for young_feature_index in range(
                young_feature_index, -1, -1
            ):  # Note: includes index 0
                young_feature = feature_group[young_feature_index]
                young_feature_begin_time, young_feature_end_time = (
                    young_feature.get_valid_time()
                )
                ...
                if young_feature_begin_time != young_time:
                    ...
                    interpolation_end_time = min(
                        young_feature_end_time, old_feature_end_time
                    )
                    ...
                    if old_time - young_time > maximum_age_difference:
                        if print_debug_output >= 2:
                            print(
                                "    ...skipping old/young feature pair - age difference {0} exceeded maximum {1}".format(
                    ...
                    stage_rotation = rotation_model.get_rotation(
                        ...
                        left_plate_id,
                        old_time,
                        anchor_plate_id=right_plate_id,
                    )
                    # If rotation not found (or is identity rotation - has no axis) then skip interpolation altogether.
                    if pygplates.FiniteRotation.represents_identity_rotation(
                        stage_rotation
                    ):
                        if print_debug_output >= 2:
                            print(
                                "    ...skipping old/young feature pair - identity stage rotation"
                            )
                        break
                    stage_rotation_pole, stage_pole_angle_radians = (
                        stage_rotation.get_euler_pole_and_angle()
                    )
                    ...
                    #
                    to_stage_pole_reference_frame_for_present_day_geometry = (
                        rotation_model.get_rotation(
                            old_time, left_plate_id, anchor_plate_id=right_plate_id
                        )
                        * inverse_present_day_rotation_for_interpolated_isochrons
                    )
                    from_stage_pole_reference_frame_for_present_day_geometry = (
                        to_stage_pole_reference_frame_for_present_day_geometry.get_inverse()
                    )
                    stage_rotation_pole_for_present_day_geometry = (
                        from_stage_pole_reference_frame_for_present_day_geometry
                        * stage_rotation_pole
                    ...
                    if print_debug_output >= 3:
                        print(
                            "    Stage pole (lat,lon) for present day geometry: {0}".format(
                                stage_rotation_pole_for_present_day_geometry.to_lat_lon()
                            )
                        )
                    ...
                    if using_interpolation_times:
                        ...
                        for young_time_index in range(len(interpolation_times)):
                            if interpolation_times[young_time_index] >= young_time:
                                ...
                                for old_time_index in range(
                                    len(interpolation_times), young_time_index, -1
                                ):
                                    if (
                                        interpolation_times[old_time_index - 1]
                                        < old_time
                                    ):
                                        ...
                                        interpolation_ratios.extend(
                                            ...
                                            min(
                                                max(
                                                    (time - young_time) * interp_denom,
                                                    0,
                                                ),
                                                1,
                                            )
                                            for time in interpolation_times[
                                                young_time_index:old_time_index
                                            ]
                                        )
                                        break
                                break
                ...
                if young_feature.get_feature_type() != COB_FEATURE_TYPE:
                    ...
                    # We can't just use the left plate ID because each mid-ocean ridge is added to its two adjacent flanks
                    # with swapped left/right plates - so we should use its reconstruction plate ID instead.
                    present_day_rotation_for_young_feature = (
                        rotation_model.get_rotation(
                            0, young_feature.get_reconstruction_plate_id()
                        )
                    )
                    # Note that we don't use 'pygplates.reconstruct' since that excludes features that don't exist at present day.
                    young_feature_geoms = [
                        present_day_rotation_for_young_feature * geom
                        for geom in young_feature_geoms
                    ]
                        ...
                        for young_feature_geom in young_feature_geoms:
                            ...
                            if old_feature.get_feature_type() == COB_FEATURE_TYPE:
                                maximum_latitude_non_overlap_radians = (
                                    maximum_cob_latitude_non_overlap_radians
                                )
                            else:
                                maximum_latitude_non_overlap_radians = (
                                    maximum_isochron_latitude_non_overlap_radians
                                )
                            ...
                            try:
                                ...
                                if print_debug_output >= 2:
                                    print(
                                        "    ...skipping old/young geometry pair - geometry(s) not polyline(s)"
                                    )
                            ...
                            if interpolated_polylines is None:
                                if print_debug_output >= 2:
                                    print(
                                        "    ...skipping old/young geometry pair - geometries do not "
                            ...
                            if using_interpolation_times:
                                ...
                                for (
                                    interpolation_index,
                                    interpolation_ratio,
                                ) in enumerate(interpolation_ratios):
                                    # The interpolated polyline associated with the interpolation time...
                                    interpolated_polyline = interpolated_polylines[
                                        interpolation_index
                                    ]
                                    interpolation_time = (
                                        young_time
                                        + interpolation_ratio * (old_time - young_time)
                                    )
                                    ...
                                    if output_scalar_types:
                                        coverage_polyline, coverage_scalars = (
                                            interpolated_polyline
                                        )
                                ...
                                if print_debug_output >= 2:
                                    ...
                                    if interpolated_polylines:
                                        print(
                                            "    ...successful interpolation - number of interpolations {0}".format(
                                ...
                                num_interpolation_intervals = (
                                    len(interpolated_polylines) - 1
                                )
                                interpolation_time_interval = (
                                    old_time - young_time
                                ) / num_interpolation_intervals
                                ...
                                for (
                                    interpolate_index,
                                    interpolated_polyline,
                                ) in enumerate(interpolated_polylines):
    ...
    interpolated_features.sort(
        key=lambda feature: (
            _get_left_right_plate_ids(feature),
            feature.get_valid_time()[0],
        )
    )
    ...
    for interpolated_feature_index, interpolated_feature in enumerate(
        interpolated_features
    ):
        interpolated_feature.set_name(
            "{0} Produced by pygplates revision {1}".format(
                interpolated_feature_index + 1, pygplates_imported_version_string
            )
        )
...
if __name__ == "__main__":
    ...
    __description__ = """Generates interpolated isochrons using continental-oceanic boundaries, isochrons and present-day ridges.
    ...
    interpolate_group.add_argument(
        "-i",
        "--interpolate_resolution_minutes",
        type=parse_positive_number,
        help=argparse.SUPPRESS,
    )
    interpolate_group.add_argument(
        "-l",
        "--interpolate_resolution_degrees",
        type=parse_positive_number,
    ...
    parser.add_argument(
        "--minimum_latitude_overlap_degrees",
        type=float,
    ...
    parser.add_argument(
        "--maximum_isochron_latitude_non_overlap_degrees",
        type=float,
    ...
    parser.add_argument(
        "--maximum_cob_latitude_non_overlap_degrees",
        type=float,
    ...
    parser.add_argument(
        "--maximum_distance_threshold_degrees",
        type=float,
    ...
    parser.add_argument(
        ...
        "not be interpolated - defaults to {0}.".format(DEFAULT_MAXIMUM_AGE_DIFFERENCE),
    )
    parser.add_argument(
        "--join_polylines_threshold_degrees",
        type=float,
    ...
    interpolate_isochrons_kwargs["tessellate_threshold_radians"] = (
        math.radians(
            args.tessellate_threshold_degrees
        )  # convert from degrees to radians
        if args.tessellate_threshold_degrees
        else None
    )
    ...
    if args.scalar_coverages:
        ...
        if unknown_scalar_coverages:
            raise TypeError(
                "Unknown scalar coverages {0}".format(unknown_scalar_coverages)
            )

        # Set scalar coverage keyword arguments.
        interpolate_isochrons_kwargs["output_scalar_spreading_asymmetry"] = (
            SCALAR_COVERAGE_SPREADING_ASYMMETRY in args.scalar_coverages
        )
        interpolate_isochrons_kwargs["output_scalar_spreading_rate"] = (
            SCALAR_COVERAGE_SPREADING_RATE in args.scalar_coverages
        )
        interpolate_isochrons_kwargs["output_scalar_full_spreading_rate"] = (
            SCALAR_COVERAGE_FULL_SPREADING_RATE in args.scalar_coverages
        )
        interpolate_isochrons_kwargs["output_scalar_spreading_direction"] = (
            SCALAR_COVERAGE_SPREADING_DIRECTION in args.scalar_coverages
        )
        interpolate_isochrons_kwargs["output_scalar_spreading_obliquity"] = (
            SCALAR_COVERAGE_SPREADING_OBLIQUITY in args.scalar_coverages
        )
        interpolate_isochrons_kwargs["output_scalar_age"] = (
            SCALAR_COVERAGE_AGE in args.scalar_coverages
        )
    ...
    output_features = interpolate_isochrons(
        ...
        interpolate,
        math.radians(
            args.minimum_latitude_overlap_degrees
        ),  # convert from degrees to radians
        math.radians(
            args.maximum_isochron_latitude_non_overlap_degrees
        ),  # convert from degrees to radians
        math.radians(
            args.maximum_cob_latitude_non_overlap_degrees
        ),  # convert from degrees to radians
        math.radians(
            args.maximum_distance_threshold_degrees
        ),  # convert from degrees to radians
        args.maximum_age_difference,
        math.radians(
            args.join_polylines_threshold_degrees
        ),  # convert from degrees to radians
        # Unpack the keyword arguments dict into keyword arguments...
        **interpolate_isochrons_kwargs,
    )

    if args.scalar_coverages and os.path.splitext(args.output_filename)[1] == ".xy":
