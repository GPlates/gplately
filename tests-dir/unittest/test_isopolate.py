#!/usr/bin/env python3
import pygplates
import sys, requests, zipfile, io, os

from gplately.lib import isopolate
from plate_model_manager import PlateModelManager

if len(sys.argv) > 1:
    # the first argument is "reconstruction time"
    ReconTime = int(sys.argv[1])
else:
    ReconTime = 0  # set to 0 if not provided
...
if len(sys.argv) > 5:
    ...
    assert plate_model
    RotFile = plate_model.get_rotation_model()
    RidgeFile = f"{age_grid_input_dir}/Global_EarthByte_GeeK07_Ridges.gpml"
    IsochronFile = f"{age_grid_input_dir}/Global_EarthByte_GeeK07_Isochrons.gpml"
    IsoCOBFile = f"{age_grid_input_dir}/Global_EarthByte_GeeK07_IsoCOB.gpml"
...
featureCollection_iso = pygplates.FeatureCollectionFileFormatRegistry()
features_iso = featureCollection_iso.read(IsochronFile)
IsochronFile_subset = pygplates.FeatureCollection()
...
# -- For the ridge file
featureCollection_ri = pygplates.FeatureCollectionFileFormatRegistry()
features_ri = featureCollection_ri.read(RidgeFile)
RidgeFile_subset = pygplates.FeatureCollection()
...
# -- For the isocob file
featureCollection_isocob = pygplates.FeatureCollectionFileFormatRegistry()
features_isocob = featureCollection_isocob.read(IsoCOBFile)
IsoCOBFile_subset = pygplates.FeatureCollection()
...
if interpolate_isochrons_kwargs["output_scalar_spreading_rate"]:
    output_scalar_types.append(pygplates.ScalarType.create_gpml("SpreadingRate"))
if interpolate_isochrons_kwargs["output_scalar_spreading_asymmetry"]:
    output_scalar_types.append(pygplates.ScalarType.create_gpml("SpreadingAsymmetry"))
if interpolate_isochrons_kwargs["output_scalar_full_spreading_rate"]:
    output_scalar_types.append(pygplates.ScalarType.create_gpml("FullSpreadingRate"))
if interpolate_isochrons_kwargs["output_scalar_spreading_direction"]:
    output_scalar_types.append(pygplates.ScalarType.create_gpml("SpreadingDirection"))
if interpolate_isochrons_kwargs["output_scalar_spreading_obliquity"]:
    output_scalar_types.append(pygplates.ScalarType.create_gpml("SpreadingObliquity"))
