...
def _reconstruct_by_topologies(time, seafloorgrid, clean=False):
    ...
    if time == np.sort(gridding_times)[-1]:
        ...
        assert (
            initial_sr == seafloorgrid.initial_ocean_mean_spreading_rate
        ), "Initial grids have not been allocated the correct initial mean ocean spreading rate."
    ...
    # ensure the initial spreading rate is still the mode in the array
        ...
        assert (
            most_common_spreading_rate == seafloorgrid.initial_ocean_mean_spreading_rate
        ), "Initial grids have not been allocated the correct initial mean ocean spreading rate."

    # Otherwise, make sure the array is not empty, and has no NaNs.
    else:
        assert (
            np.unique(np.isnan(spreading_rate_data))[0] is False
        ), "Some spreading rates in the {} Ma gridding input have been ascribed a NaN value.".format(
            time
        )
