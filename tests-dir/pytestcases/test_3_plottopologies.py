...
def test_plot_topology_adds_artists(gplot):
    """Test that plot_topology adds artists to the axes (gap-free topology helper)."""
    ccrs = pytest.importorskip("cartopy.crs")

    fig = plt.figure(figsize=(8, 4))
    ax = fig.add_subplot(111, projection=ccrs.PlateCarree())

    artists_before = len(ax.collections) + len(ax.lines) + len(ax.patches)
    gplot.plot_topology(ax)
    artists_after = len(ax.collections) + len(ax.lines) + len(ax.patches)

    assert (
        artists_after > artists_before
    ), "plot_topology did not add any artists to the target axes."

    plt.close(fig)


def test_plot_topology_skip_layer(gplot):
    """Test that passing False for a layer skips it in plot_topology."""
    ccrs = pytest.importorskip("cartopy.crs")

    # Draw with backbone only (no ridges, transforms, or trenches)
    fig, (ax1, ax2) = plt.subplots(
        1, 2, subplot_kw={"projection": ccrs.PlateCarree()}, figsize=(12, 4)
    )
    ...
    collections_backbone_only = len(ax1.collections) + len(ax1.lines)

    gplot.plot_topology(ax2)
    collections_full = len(ax2.collections) + len(ax2.lines)

    # Full topology (backbone + ridges + trenches) should add more artists than
    # backbone alone.
    assert (
        collections_full >= collections_backbone_only
    ), "plot_topology with all layers should have at least as many artists as backbone-only."
